...
public partial class Html2RFCOut  {
    ...
    public void Write (TextSegmentOpen Text) {
        ...
        switch (Text.Tag) {
            ...
            case "xref": {
                var id = Text.Attributes?[0].Value;
                var (text, textid) = GetReference(id);
    ...
    (string, string) GetReference(string anchor) {
        ...
        if (target.Section != null) {
            ...
            return ($"subsection {number}", number);
            }
        ...
        if (textblock is Figure figure) {
            return (figure.SectionText, figure.NumericID);
            }
        if (textblock is Table table) {
            return (table.SectionText, table.NumericID);
            }
        if (textblock is P p) {
            ...
            return ($"section {number} paragraph {p.NumericID}", number);
            }
    ...
    public void Write (TextSegmentClose Text) {
        if (Text.Open.Trailer != null) {
            Output.Write(Text.Open.Trailer);
            }
